...
| `GET` | `/api/results/{search_name}/{run_date}` | Full run document; `feedback` decoded to `{url: text}`. Same visibility gate as above (404) — a run whose search config has been deleted (orphaned) is admin-only. Within that gate, no further redaction is applied: on a public search, any viewer (including anonymous) sees the full run data, including `feedback`, `pinned_finds`, `example_urls`, and the `config_snapshot`'s `description`/`feedback_notes`/`avoid_shops`. Private searches are unaffected — they still 404 for anyone but the owner or an admin via the gate above. `pinned_finds` and `example_urls` are both overridden with the *live* value from `shop_searches` (not the run's frozen `config_snapshot`), so pin/unpin actions and reference-product edits show up immediately regardless of which run is being viewed |
...
- **Google OAuth (`sa_session` cookie):** JWT signed with `SESSION_SECRET`. Set on `/auth/callback` after Google verifies identity. User record stored in Firestore `users` collection; `role` field determines access. `BOOTSTRAP_ADMIN_EMAIL` is promoted to `admin` on first login. New-user registration is currently closed: `/auth/callback` only creates a `users` doc for an email that already has one, or for `BOOTSTRAP_ADMIN_EMAIL`; any other new email is redirected to `/?blocked=registration_closed` instead of signing in. Existing users are unaffected.
